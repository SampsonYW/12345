...
func _physics_process(delta: float) -> void:
	...
	var speed: float = base_speed * (sprint_multiplier if is_sprinting else 1.0)
...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed("sprint") and cooldown_timer <= 0.0 and not is_sprinting:
		is_sprinting = true
...
func get_sprint_cooldown_ratio() -> float:
	if sprint_cooldown <= 0.0:
		return 0.0
	return clampf(cooldown_timer / sprint_cooldown, 0.0, 1.0)
