# container_search_overlay.gd
# HUD 的容器搜索弹层视图，包含左侧背包和右侧容器槽位
...
func setup(hud: Control) -> void:
	...
	right.add_child(_hud._make_overlay_title("容器"))
	...
	_hud._search_feedback_label = _hud._make_hint_label("")
	_hud._search_feedback_label.name = "SearchFeedback"
	right.add_child(_hud._search_feedback_label)
	
	right.add_child(_hud._make_hint_label("右键已发现物品可快速转移到背包；右键背包格子可存入容器。"))
