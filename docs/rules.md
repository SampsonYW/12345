...
> **禁止**：在项目根目录随意放置脚本或场景文件（`game.gd` 作为历史遗留暂时保留，后续应迁入 `scripts/`）。
...
项目采用模块化协作。每个开发者拥有明确的文件边界和对外接口，跨模块协作优先通过 Autoload、信号、Group、Resource 和公有函数完成。
...
- 修改某个模块前，先确认该模块 owner。不是 owner 的人只改调用侧，不直接改 owner 内部实现。
- 需要改对方模块接口时，先在站会或 PR 描述中说明“改了什么接口、谁会受影响、如何验证”。
- HUD 只展示状态，不承担玩法逻辑。玩法逻辑放回 Player / Inventory / Enemy / Manager owner 模块。
...
- 一个分支尽量只覆盖一个 owner 模块。确实需要跨模块时，在 PR 描述中列出涉及的 owner 和接口变化。
...
4. **识别 owner**: 根据 §1.4 确认目标模块 owner 和禁止改动区域
...
6. **尊重边界**: 非 owner 文件只改调用侧；需要改 owner 内部实现时，说明原因和影响面
...
1. 每天前半段各自完成 owner 模块的小闭环。
2. 中途同步一次接口变化，尤其是公有函数、信号、Group、Resource 字段。
3. 当天最后 30-45 分钟只做联调：Dev A 接 HUD/状态，Dev B 接物品/地图，Dev C 接敌人/压力。
4. 如果出现跨域 Bug，先判断是调用方错误还是 owner 内部错误；调用方错误由调用方修，内部逻辑由 owner 修。
...
- 场景文件（`.tscn`）的结构修改**只在 Godot 编辑器中进行**
- 避免手动编辑 `.tscn` 文本（除非你完全理解格式）
...
| 文档 | 路径 | 用途 | 维护者 |
|------|------|------|--------|
| 游戏设计 | `docs/design.md` | 核心玩法设计（权威来源） | 设计师 + 全员 |
| 技术实现 | `docs/implementation.md` | 代码架构 & 伪代码 | 开发者 |
| 冲刺计划 | `docs/sprint.md` | 每日任务分配 | 全员 |
| 资源清单 | `docs/art_audio_resource_checklist.md` | 美术、音效、音乐交付清单 | Designer D + 全员 |
