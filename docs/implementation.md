...
- **数值**：`elapsed_time`、`player_erosion`、`zone_erosion_multiplier`、`max_weight`、`max_erosion`、`kill_count`
...
| `AfterglowMap / ExpeditionMap` | `current_state / ui_blocking_input / begin_expedition / zone_erosion_multiplier` | 每帧 |
...
为避免 32 个敌人同帧密集计算导致卡顿，各 AI 子系统都做了定时节流：
...
所有定时器在 `_ready()` 中用 `randf() * INTERVAL` 初始化随机偏移，使 32 个敌人的寻路和视线检测分散到不同帧，避免同帧集中触发。注意：Billboard 朝向更新已改回每帧执行，因为位置跟随若被节流会导致 UI 相对于角色的移动发生明显掉帧。
...
1. **未破解**：玩家长按 E 进入 `_is_cracking`，`_crack_progress` 按 `get_crack_duration()` 推进（侵蚀越高越慢），同时在容器上方显示一个 3D 进度条（通过代码动态创建 `SubViewport` 和 `Sprite3D` 实现）。完成后 `_complete_crack()` 隐藏进度条，调 `NoiseManager.emit_noise(LOW)` + `open_container()` + 广播 `cracked(self)`。
...
- 每帧累积 `_spawn_budget += spm * delta / 60.0`，`minf` 钳位到 `MAX_SPAWN_BUDGET = 5.0` 防止无限积累；超过 1.0 就 `spawn_enemy()` 一次，单帧上限 2 个。每次实际生成后设置 `SPAWN_COOLDOWN = 0.15s` 冷却间隔，确保连续生成之间间隔 2-3 帧，给寻路系统留初始化时间。
- **初始刷怪分帧**：`seed_initial_enemies()` 收集所有初始 spawn 数据到队列，通过 `_deferred_spawn_queue()` 每帧最多实例化 2 个敌人（`await get_tree().process_frame`），避免开局 10+ 个敌人同帧生成导致卡顿。
...
3. `update(delta)` 每帧由 Game3D 调用，做两件事：`_update_risk_label`（把当前 zone 名/风险传给 HUD，**并向 GameManager 写入当前 zone 的 zone_erosion_multiplier**）和 `_update_container_interactions`（找玩家附近 ≤ 3.2m 的容器，显示"长按 E 开启"或"E 搜索"）。
...
NavAgent 内部按需重算 path（玩家移动时 target_position 变 → 自动 repath）。`_nav_move_toward` 现已做节流优化（见 §6.4）：每 `NAV_UPDATE_INTERVAL`（0.2s）才更新一次 `target_position` 和 `get_next_path_position()`，中间帧沿缓存方向 `_cached_nav_direction` 继续移动，大幅减少 32 个敌人同时追击时的寻路开销。
...
| 参数 | 当前值 | 定义位置 |
|------|--------|----------|
| `GameManager.EROSION_RATE` | 0.0417/s（≈ 2.5%/60s） | `game_manager.gd` const |
...
| SpawnManager MAX_SPAWN_BUDGET | 5.0 | `spawn_manager.gd` const |
| SpawnManager SPAWN_COOLDOWN | 0.15s | `spawn_manager.gd` const |
| Enemy NAV_UPDATE_INTERVAL | 0.2s | `enemy_3d.gd` const |
| Enemy VISION_CHECK_INTERVAL | 0.15s | `enemy_3d.gd` const |
| Enemy OBSTACLE_CHECK_INTERVAL | 0.1s | `enemy_3d.gd` const |
