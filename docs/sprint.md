...
| **Day 4** | 信号弹 UI 状态、撤离/死亡状态 HUD、必要的输入提示 | 可视视野（视野锥 + 近距圆 + 障碍遮挡）、弹药箱/医疗容器补给逻辑 | `SpawnManager` 时间刷怪、信号弹全图噪音、刷怪强度 ×3、`Extraction` 等待/登车/成功流程 |
...
| 地图模板设计（纸上/Tiled 规划 20-30 屏布局） | Dev B | 地图布局草稿（区分高/低价值区） |
...
| 侵蚀影响视野锥射程（可视视野） | Dev B | 视野锥 + 近距 360° 圆，侵蚀越高视野锥越短 |
| 障碍遮挡 + 隐藏盲区敌人 | Dev B | 视野外的敌人/掉落物 `visible = false`，墙后不可见 |
...
| 风险 | 触发条件 | 应对 |
|------|----------|------|
| 可视视野来不及做 | Day 4 仍未实现 | 降级为固定大视野无遮挡 |
