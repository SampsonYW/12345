...
<style>
  table { font-size: 11px; word-break: break-word; }
  th { font-size: 11px; background-color: #1a1a2e; color: #e0e0e0; padding: 4px 6px; }
...
| 模块 | 资源 | 当前状态 | 对接模块 |
|------|------|----------|----------|
| 主角 | 3D 占位 Mesh（移动/静止） | [OK] BoxMesh 占位 | scenes/player_3d.tscn |
| 武器 | 子弹 3D 发光体 | [OK] 小 BoxMesh + 发光材质 | scenes/bullet_3d.tscn |
| 敌人 | 巡逻型/休眠型 3D 占位 Mesh | [OK] BoxMesh + 警戒条/HP条 | scenes/patrol_enemy_3d.tscn / dormant_enemy_3d.tscn |
| 容器 | 通用容器 3D Mesh（破解前/中/后三色） | [OK] BoxMesh + 动态颜色材质 | scenes/container_3d.tscn |
| 物品 | 地面拾取物（4 类颜色区分） | [OK] 小 BoxMesh + 颜色代码 | scenes/item_pickup_3d.tscn |
| HUD | HP条/侵蚀条/弹药/负重/分数/信号弹 | [OK] Godot 控件程序化绘制 | scripts/ui/hud.gd |
| HUD | 破解读条进度条 | [OK] 程序化 ColorRect 条 | scripts/ui/hud.gd |
| HUD | 8 格背包槽 + 物品颜色标识 | [OK] Panel + ColorRect | scripts/ui/hud.gd |
| HUD | 开始界面/结算界面 | [OK] ColorRect 程序化 UI | scripts/ui/hud.gd |
| VFX | 信号弹标记 + 撤离信标 + 母车到达标记 | [OK] 3D 占位 marker | Extraction 系统 |
| VFX | 侵蚀导致视野缩小（shader） | [OK] fog shader（仅侵蚀视野缩小） | scripts/systems/fog_of_war.gd |
| 地图 | 障碍物（废墟/残骸/管道） | [OK] 3D 占位 Mesh 已放置 | scenes/expedition_map.tscn |
| 地图 | 边界（电磁障壁占位） | [OK] 3D 占位 Mesh | 地图边界 |
...
| 主角 | 主角移动动画 | 6-8 帧 | 360 度移动时播放 | scripts/player/player_3d.gd |
...
| 敌人 | 敌人警戒值头顶条 | [OK] 已实现（程序化 3D bar） | 警戒值升高可见 | scripts/enemies/enemy_3d.gd |
...
| VFX | 信号弹发射特效 | 4-8 帧 | 按 Q 发射信号弹 | Extraction 系统 |
| VFX | 撤离等待标识循环特效 | 1 套 | 撤离等待位置视觉 | Extraction 系统 |
| VFX | 母车剪影/低模 + 到达提示 | 1 个剪影 + 1 套提示特效 | 登车成功点 | Extraction 系统 |
...
| 容器 | 搜索揭示完成 | 1 个 | 物品格揭示完毕 | 搜索 UI 揭示 |
...
| 撤离 | 信号弹发射 | 1 个 | 发射撤离信号 | signal_flare 按键 |
...
| 刷怪 | 屏幕边缘脉冲方向音效 | 1-2 个 | 敌人从某方向涌入 | SpawnManager.spawn_occurred |
| 事件 | 容器警报 | 1 个 | 破解触发警报 | EventManager |
| 事件 | 紧急补给出现 | 1 个 | 临时补给点出现 | EventManager |
| 事件 | 电磁脉冲 | 1 个 | 区域断电/敌人觉醒 | EventManager |
| 事件 | 封锁区域开启 | 1 个 | 通道打开 | EventManager |
| 事件 | 敌人潮汐预警 | 1 个 | 某方向大量敌人涌入 | EventManager |
...
| Day 7 | 美术 placeholder 替换（主角/敌人/容器 Mesh 替换为纸片 Sprite）、侵蚀视觉叠加、提交封面图/截图 | 打磨观感、准备提交 |
